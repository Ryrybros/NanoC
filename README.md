...
It is not possible to address an expression, only a variable: &var.

Since pointers are considered as dynamic arrays (and vice versa), it is possible ot access the value of ptr+(8*i) with ptr[i]. This means that the access to the memory is not safe.
It is not possible to declare a pointer of an array: int[]*.
...
It is not possible to address an element of an array (&T[i]), it should be done separately: a = T[i]; &a.
